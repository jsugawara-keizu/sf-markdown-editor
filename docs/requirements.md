...
- Task 作成時、リマインダーの利用On/Offと日時（`IsReminderSet` / `ReminderDateTime`）を行ごとに設定できる。日時は既定で期日（`ActivityDate`）当日の 09:00（ローカル時刻）が自動入力され、期日変更時は未編集の間は追従する（詳細・既定値の考え方は [docs/backlog/10_user-feedback.md](backlog/10_user-feedback.md) の項目2を参照）。
...
ユーザーヒアリング等で得た背景・改善案を含む詳細な要望メモは [`docs/backlog/`](backlog/README.md) を参照。
