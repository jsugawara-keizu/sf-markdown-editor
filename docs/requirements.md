...
Salesforce のレコードページ上で、任意の長文テキスト項目（Long Text Area）を Markdown で編集・表示できるようにする。対応ドキュメント（`hk.issue` の `TaskMng__c.Markdown__c` 等）を Salesforce の外に持ち出さずに、レコードに紐づけたまま編集・レンダリング・スライド表示・タスク化まで行えることを目的とする。
...
| 3   | `<details>`/`<summary>` の開閉状態を示す視覚的インジケーター | 未着手                            |
