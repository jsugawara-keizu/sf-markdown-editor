...
| 3   | `<details>`/`<summary>` の開閉状態を示す視覚的インジケーター | ✅ 再対応済み（2026-08-25）。初版（5px・文字色と同系統の三角形）が「表示が分かりにくい」とユーザーヒアリングで判明したため、`markdownViewer.css` のマーカーをブランドカラー（`--slds-g-color-brand-base-50`）のシェブロン（0.6em、45度回転）に変更し視認性を強化。hk.issue にデプロイ済み（実機での見え方はユーザー確認待ち） |
