...
トグルバーの切り替えボタンでスライドモード ↔ ドキュメントモード（通常 Markdown プレビュー）を切り替えます。ドキュメントモードでは `c-markdown-viewer` をそのまま表示します。
...
スライドモード時にトグルバーに全画面表示ボタン（`utility:expand`）が表示されます。クリックすると `position: fixed; inset: 0; z-index: 9999` の CSS オーバーレイで全画面表示になります。再クリックまたは `Esc` キーで元に戻ります。

> **注意:** LWS (Lightning Web Security) はブラウザの `requestFullscreen()` API をブロックするため、CSS オーバーレイによる擬似フルスクリーンを採用しています。

### 6.8 カスタムラベル（marpViewer）
