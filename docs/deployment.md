...
- 本リポジトリを開発元とする LWC（`markdownEditor` 等）は、hk.issue を含む複数の Sandbox 環境へ同一コンポーネントをデプロイする運用になっている。**hk.issue でのメタデータ変更は必ずこのリポジトリ側で行い、hk.issue リポジトリの `force-app/` を直接編集してデプロイしない**（hk.issue 側の CLAUDE.md に記載の例外運用）。デプロイ後は、hk.issue 側の `force-app/main/default/` にも最新状態を反映してコミットし、乖離を防ぐこと。
...
`MarkdownEditorViewer` 権限セットには、デプロイ先環境固有のオブジェクト・項目への FLS は**意図的に**含まれていない（[development.md](development.md#権限セット運用方針) 参照）。プロファイルまたは環境専用の権限セットで、対象項目（例: `TaskMng__c.Markdown__c`）への読み取り・編集権限を個別に付与すること。
...
## 開発元リポジトリからの複数 Sandbox 展開（hk.issue 等）

本リポジトリは、hk.issue をはじめとする複数の Sandbox 環境向けの「開発元リポジトリ」として運用されている（2026-08〜）。
...
3. デプロイ後、本リポジトリの最新状態を各 Sandbox リポジトリ（例: hk.issue）の `force-app/main/default/` にも `cp` 等で反映してコミットし、乖離を防ぐ。
